...
def getXmlScores (abc_string, skip=0, num=1, rOpt=False, tOpt=False, bOpt=False): # added by David Randolph
    xml_strings = []
    ...
    for itune, tune in enumerate (tunes):
        ...
        try:                                # convert string abctext -> file pad/fnmNum.xml
            score = mxm.parse (tune, rOpt, tOpt, bOpt)
...
if __name__ == '__main__':
    ...
    for X, fnmext in enumerate (fnmext_list):
        ...
        if ext.lower () not in ('.abc'):
            info ('skipped file %s, it should have extension .abc' % fnmext)
            continue
        ...
        skip, num = options.m
        xml_strings = getXmlScores (abctext, skip, num, options.r, options.t, options.b)
