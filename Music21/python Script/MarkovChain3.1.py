...
'''
Terzo Metodo implementativo che sfrutta la funzione calcHarmony per aggiungere  due sequenze di note all'originale, ognuna ad un intervallo desiderato
'''
...
startingNoteHarmony5 = calcHarmony(startingNote, 'quinta')
startingNoteHarmony3 = calcHarmony(startingNote, 'ottava')

newScore1.append(note.Note(startingNote, type=noteTypeDurations[0]))
newScore2.append(note.Note(startingNoteHarmony5, type=noteTypeDurations[0]))
newScore3.append(note.Note(startingNoteHarmony3, type=noteTypeDurations[0]))
...
for noteIndex in range(1, 60):
	...
	for index in range(12):
		...
		if randomNumber <= temp:
			...
			newScore1.append(note.Note(nextNote, type=duration)) #aggiungola nota trovata allo spartito

			harmony5 = calcHarmony(nextNote, 'quinta')
			newScore2.append(note.Note(harmony5, type=duration))

			harmony3 = calcHarmony(nextNote, 'ottava')
			newScore3.append(note.Note(harmony3, type=duration))
